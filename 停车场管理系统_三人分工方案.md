...
```
综合性实验/
├── ds.h              # Person A: 数据结构定义 + 函数声明
├── ds.c              # Person A: 栈/队列/哈希表实现
├── parking.h         # Person B: 停车场业务函数声明
├── parking.c         # Person B: 入场/出场/查找/排队
├── fee.h             # Person B: 计费模块声明
├── fee.c             # Person B: 计费逻辑
├── stats.h           # Person C: 统计模块声明
├── stats.c           # Person C: 统计实现
├── ui.h              # Person C: 菜单界面声明
├── ui.c              # Person C: 菜单 + 交互实现
├── file_io.h         # Person C: 文件读写声明
├── file_io.c         # Person C: 数据持久化
├── main.c            # Person C: 程序入口
└── Makefile          # (可选) 编译脚本
```

---

## 五、Person A — 数据结构层（底层基础设施）
...
C 调用 A 的：`ds_init`, `ds_destroy`, `stack_is_empty`, `stack_size`, `queue_count`, `ht_find`

C 调用 B 的：`vehicle_entry`, `vehicle_exit`, `find_vehicle`, `find_in_queue`

C 自己实现：所有 `stats_*`, `ui_*`, `save_data`, `load_data`, `main`
...
| 人员 | 任务 |
|------|------|
| **A** | 完成 `ds.h` 和 `ds.c` —— 所有结构体定义 + 栈/队列/哈希表全部实现 |
| **C** | 完成 `ui.h` 和 `ui.c` 的菜单框架 + `file_io.h`/`file_io.c` 框架 |
| **B** | 阅读 ds.h 了解接口，设计 parking.c 和 fee.c 的内部逻辑 |

### 第二轮（并行开发）

| 人员 | 任务 |
|------|------|
| **A** | 自测 ds.c 所有函数（写测试主函数验证栈/队列/哈希表） |
| **B** | 基于 ds.h 开发 `parking.c` + `fee.c` 全部业务逻辑 |
| **C** | 完成 `stats.c` + `file_io.c` 全部实现 + `main.c` |
...
| # | 功能 | 说明 | 负责 |
|---|------|------|------|
| 1 | 系统初始化 | 创建 60 个空闲车位 | **A** |
| 2 | 空闲栈 | pop/push 操作正确 | **A** |
| 3 | 哈希表 | 插入/查找/删除 O(1) | **A** |
| 4 | 等待队列 | 入队/出队 FIFO | **A** |
| 5 | 入场登记 | 有空位分配，无空位排队 | **B** |
| 6 | 出场结算 | 释放车位，计算费用 | **B** |
| 7 | 排队调度 | "出一个进一个" | **B** |
| 8 | 实时查找 | 查找在场 / 排队车辆 | **B** |
| 9 | 计费逻辑 | 免停 + ceil小时 + 封顶 | **B** |
| 10 | 车位状态图 | ASCII 图显示占用/空闲 | **C** |
| 11 | 收入统计 | 总收入 / 今日 / 本月 | **C** |
| 12 | 交易明细 | 历史记录分页显示 | **C** |
| 13 | 楼层热度 | 各层周转率排名 | **C** |
| 14 | 峰值时段 | 各小时入场统计 | **C** |
| 15 | 数据持久化 | 二进制文件保存/加载 | **C** |
| 16 | 菜单界面 | 循环菜单，各功能入口 | **C** |
| 17 | 内存管理 | 无内存泄漏 | **A,C** |
